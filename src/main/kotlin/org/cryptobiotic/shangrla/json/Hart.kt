package org.cryptobiotic.shangrla.json
...
class Hart {
    ...
    companion object {
        val WRITE_IN = "WRITE_IN"
        val NO_CANDIDATE = "NO_CANDIDATE"

        /*
        fun prep_manifest(manifest: pd.DataFrame, max_cards: Int, n_cvrs: Int) {
            /*
            ...
            Parameters:
            ----------
            manifest: DataFrame; should contain the columns; upper bound on the number of cards cast
            n_cvrs: Int; number of CVRs

            Returns:
            --------
            manifest: pandas DataFrame
                original manifest with additional column for cumulative cards and, if needed,
                an additional batch for any phantom cards
            manifest_cards: Int; the total number of cards in the manifest
            phantoms: Int; the number of phantom cards required
            */
            val cols = listOf("Container", "Tabulator", "Batch Name", "Number of Ballots")
            assert set (cols).issubset(manifest.columns), "missing columns"
            val manifest_cards = manifest["Number of Ballots"].sum()
            ...
            if (manifest_cards < max_cards) {
                ...
                warnings.warn("manifest does not account for every card; appending batch of {phantoms} phantom cards to the manifest"
                )
                val r = { "Container": None, "Tabulator": "phantom", "Batch Name": 1, "Number of Ballots": phantoms }
            }
            manifest = pd.concat([manifest, pd.DataFrame([r])])
            manifest["cum_cards"] = manifest["Number of Ballots"].cumsum()
            for (c in listOf("Container", "Tabulator", "Batch Name", "Number of Ballots")) {
                manifest[c] =
                    manifest[c].astype(str) // <-why is this a str instead of a float? weird behavior when e.g. summing to get total ballots
            }
        ...
        fun read_cvr(cvr_string: String? = null): CVR {
            /*
            read a single Hart CVR from XML
        ...
        fun read_cvrs_directory(cvr_directory: String): List<CVR> {
            /*
            ...
            */
            ...
            for (file in [f for f in os.listdir(cvr_directory) if f.endswith(".xml") {
                val cvr_path = cvr_directory + "/" + file
        ...
        fun read_cvrs_zip(cvr_zip: String, size: Int? = null): List<CVR> {
            /*
            ...
            */
            ...
            val cvr_object = read_cvr(raw_string)
            cvr_list.add(cvr_object)
        ...
        fun sample_from_manifest(manifest: object = None, sample: list=None)
        {
            /*
        ...
        */
        ...
        fun sample_from_cvrs(cvr_list: list, manifest: list, sample: np.array) {
            /*
